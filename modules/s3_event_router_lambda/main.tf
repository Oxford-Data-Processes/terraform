...
module "s3_event_router_lambda" {
  ...
  function_name  = "${var.project}-s3-event-router-lambda"
  create_package = false
  image_uri      = "${var.aws_account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${var.project}-s3-event-router-lambda-image:${var.version_number}"
  package_type   = "Image"
...
resource "aws_s3_bucket_notification" "s3_event_router" {
  ...
  lambda_function {
    ...
    lambda_function_arn = "arn:aws:lambda:${var.aws_region}:${var.aws_account_id}:function:${var.project}-s3-event-router-lambda"
  }
